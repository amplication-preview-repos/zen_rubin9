/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { DbcCategoryWhereUniqueInput } from "./DbcCategoryWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { DbcCategoryUpdateInput } from "./DbcCategoryUpdateInput";

@ArgsType()
class UpdateDbcCategoryArgs {
  @ApiProperty({
    required: true,
    type: () => DbcCategoryWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => DbcCategoryWhereUniqueInput)
  @Field(() => DbcCategoryWhereUniqueInput, { nullable: false })
  where!: DbcCategoryWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => DbcCategoryUpdateInput,
  })
  @ValidateNested()
  @Type(() => DbcCategoryUpdateInput)
  @Field(() => DbcCategoryUpdateInput, { nullable: false })
  data!: DbcCategoryUpdateInput;
}

export { UpdateDbcCategoryArgs as UpdateDbcCategoryArgs };
